...
# We will use the same training and test set, as created in the 'a' and 'b' parts (MATLAB)
# t and tr are sets of the indices
...
# The indices are used to compute the training and test data sets
ytrain = [y[i-1] for i in tr]
ytest = [y[i-1] for i in t]
Xtrain = [X[i-1] for i in tr]
Xtest = [X[i-1] for i in t]

# This is the parameter string that will be used to generate the model
# '-s 13' selects L1-loss Support Vector Regression, '-v 10' specifies 10 fold cross-validation
# '-c xx' selects the value of the hyperparameter, C in the model. We will append this value later
paramstr = '-s 13 -v 10 -q -c '
# s is the range of values of C for which we will use cross-validation
lim = 50
#s = [str(i) for i in range(0.001, 15, 0.1)]
...
# Sorting the C's based on least mean square error using cross-validation
#for entry in cvlist:
	#print entry[0],',',
#print ''
#for entry in cvlist:
...
# Now using the model with hyperparameter, C to predict the data from the test set
